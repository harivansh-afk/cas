...
using the same two paths as the host configuration. With a checkout and Nix on
the target:

```sh
nix develop --command cargo run --locked -p cas-harness -- check-disks \
  /dev/disk/by-id/OS_DISK /dev/disk/by-id/TEST_DISK
```
...
The template installs experiment tools, `casctl`, and `cas-harness`. It does not start workloads,
