# Content, not lineage — research spec v5

v5 (2026-09-01, restructure): the census is the paper. Two hypotheses (H1 the cross-lineage curve against fleet age on real fleets; H2 the aligned/CDC-only split), no H3. Phase 1 census, six weeks, at least two real fleets hashed on site under a donor protocol, synthetic fleets as controls, model and Nix corpora, containers dropped. Phase 2 prices the aligned tier on four stock backends (XFS raw, ZFS ≥ 2.3 fast dedup, dm-vdo, duperemove) with transfer and cache as headline metrics and latency parity as precondition. Phase 3, the vhost-user-blk CDC daemon, is conditional on a week-6 gate (block-capturable at 4K ≥ 90% of cross-lineage on VM corpora cancels it), two weeks, one map, numbers labeled preliminary. Distribution reduced to one paragraph. Gates G1–G5 and the cut order rewritten to match. The v4 text below is retained as history.

# Chunk pointers, not block pointers — research spec v4 (history)
