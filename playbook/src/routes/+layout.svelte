<script lang="ts">
	...
	// Route ids are stable under prerendering, where `base` is relative.
	function current(): string | null {
		const m = page.route.id?.match(/^\/(0[0-6])$/);
		return m ? m[1] : null;
	}

	const inDeck = $derived(page.route.id === '/updates/[slug]');
	...
	function onkeydown(e: KeyboardEvent) {
		...
		const cur = current();
		if (cur === null && page.route.id !== '/') return;
