...
<PageHead num="00" />
<p class="lede">
	<mark>A hash that is the same on every host lets a fleet move, store, and cache each unique chunk as one thing. This study builds a block backend on that property and measures, on two hosts, what it provides and what it costs.</mark><br />
	In OpenZFS and dm-vdo the hash of a block is a key in a table that belongs to one pool. The block is still addressed by its location, and nothing the table knows leaves the host.<br />
...
<p>
	Three things follow, and each is measured. A guest is provisioned or migrated by moving its manifest, because the chunks it names already exist at their owners. Each unique chunk is stored k times across the fleet instead of once per host. A chunk is served from whichever host holds it in memory.<br />
...
<p>
	...
	In fleet class the daemon also sends the bytes themselves to one fixed peer (not the manifest, since the manifest names bytes that exist nowhere else yet) and acknowledges after both this host's fdatasync and the peer's. Fsynced bytes therefore survive the loss of this host.<br />
...
<p>
	...
	The transfer bound is by design, since nothing else is sent. The 55% is one copy of the unique set instead of two, because guests cloned from one image give the two pools nearly the same unique set, plus five points for headers, manifests, and leaks.<br />
...
<p>
	...
	For a 4 KiB read at QD1 whose chunk is not in the local cache:<br />
	Served from the owner's memory, guest-visible latency is lower than the same read served by the daemon from its own NVMe, over TCP and over RDMA.<br />
	Served from the owner's NVMe, it is at most 40% over the local read on TCP and 15% on RDMA.<br />
