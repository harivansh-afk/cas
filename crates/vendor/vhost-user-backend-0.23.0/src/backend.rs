...
use vhost::vhost_user::message::{
    ...
    VhostUserShMemConfig, VhostUserSharedMsg, VhostUserInflight,
};
...
pub trait VhostUserBackend: Send + Sync {
    ...
    fn get_inflight_fd(&self, _inflight: &VhostUserInflight) -> Result<(VhostUserInflight, File)> {
        Err(std::io::Error::new(
            ...
            "inflight metadata is unsupported",
        ))
    }

    /// Adopt metadata retained by the frontend; the backend owns the received file.
    fn set_inflight_fd(&self, _inflight: &VhostUserInflight, _file: File) -> Result<()> {
        Err(std::io::Error::new(
            std::io::ErrorKind::Unsupported,
            "inflight metadata is unsupported",
        ))
...
impl<T: VhostUserBackend> VhostUserBackend for Arc<T> {
    ...
    fn set_inflight_fd(&self, inflight: &VhostUserInflight, file: File) -> Result<()> {
        self.deref().set_inflight_fd(inflight, file)
    }
...
impl<T: VhostUserBackendMut> VhostUserBackend for RwLock<T> {
    ...
    fn get_inflight_fd(&self, inflight: &VhostUserInflight) -> Result<(VhostUserInflight, File)> {
        self.write().unwrap().get_inflight_fd(inflight)
    }

    fn set_inflight_fd(&self, inflight: &VhostUserInflight, file: File) -> Result<()> {
        self.write().unwrap().set_inflight_fd(inflight, file)
    }
