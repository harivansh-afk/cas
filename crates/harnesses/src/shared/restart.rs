...
pub(super) fn restart(
    ...
    evidence::write_json(
        ...
        &serde_json::json!({
            ...
            "at_utc":crate::host::utc_now()?, "guest_identities":before,
        }),
    )?;
    ...
    *host = start_host(args, build, &replacement, sockets, "retained")?;
    let after = guests
        .iter()
        .map(|(_, path, _)| Identity::observe(path))
        .collect::<io::Result<Vec<_>>>()?;
...
pub(super) fn verify_restart(output: &Path) -> io::Result<()> {
    ...
    for index in 0..2 {
        ...
        if !directory.join("ready").is_file() || !directory.join("resume").is_file() {
            return Err(io::Error::other(
